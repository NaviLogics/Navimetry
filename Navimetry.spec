...
hiddenimports = [name for name in dict.fromkeys(hiddenimports) if ".tests" not in name and not name.endswith(".tests")]

a = Analysis(
    ["main.py"], pathex=["."], binaries=binaries, datas=datas, hiddenimports=hiddenimports,
    hookspath=[], hooksconfig={}, runtime_hooks=["runtime_hook.py"], runtime_tmpdir=None,
    excludes=["matplotlib.tests", "scipy.tests", "pytest"], noarchive=False,
)
pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)
executable = EXE(
    pyz, a.scripts, [], exclude_binaries=True, name="Navimetry", debug=False,
