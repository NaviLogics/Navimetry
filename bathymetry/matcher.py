...
def _alignment_score(csv_obs:pd.DataFrame,sonar:pd.DataFrame,gpi:pd.DataFrame,offset:int)->tuple[float,int]:
    if sonar.empty or gpi.empty:return 0.0,0
    rows=np.arange(len(csv_obs)); sidx=rows+offset; valid=(sidx>=0)&(sidx<len(sonar))
...
def match_csv_to_klf(csv_obs:pd.DataFrame,global_position:pd.DataFrame,sonar_cycles:pd.DataFrame|None=None,gps_raw:pd.DataFrame|None=None,attitude:pd.DataFrame|None=None,estimator_status:pd.DataFrame|None=None,system_time:pd.DataFrame|None=None)->tuple[pd.DataFrame,dict]:
    ...
    for offset in range(-3,4):
        score,n=_alignment_score(csv_obs,sonar_cycles,global_position,offset); candidates.append((score,n,offset))
    ...
    rows=np.arange(len(csv_obs)); sidx=rows+best_offset; mapped=(sidx>=0)&(sidx<len(sonar_cycles))
