...
def evaluate_triangles(metrics: pd.DataFrame,xy: np.ndarray,faces: np.ndarray,config,point_spacing_m: float,effective_geometry: dict,track_geometry: dict) -> tuple[np.ndarray,dict]:
    ...
    Adjacent-line triangles are accepted from survey topology + anisotropic spans rather
    than isotropic triangle shape. This avoids artificial holes between legitimately
    neighbouring single-beam survey lines while retaining strict rejection of
    non-adjacent bridges, multi-line bridges, oversized spans and unsupported turns.
    """
    ...
    topology_counts={}; membership_info={"available":False}; adaptive_angle_bypass=pd.Series(False,index=metrics.index)
    ...
    if survey_aware:
        ...
        if membership_available:
            ...
            topology_ok=pd.Series(base_ok|endpoint_ok|transition_ok,index=metrics.index)
            # Delaunay triangles spanning two adjacent survey lines can be long/slender
            # solely because along-track sampling is much denser than line spacing.
            # For those triangles the finite corridor topology and anisotropic span
            # limits are stronger support evidence than max interior angle.
            adaptive_angle_bypass=pd.Series((topology=="adjacent_lines") & cross_ok.to_numpy(bool) & along_ok.to_numpy(bool),index=metrics.index)
        ...
        effective_angle_ok=angle_ok|adaptive_angle_bypass
        mask=cross_ok&along_ok&effective_angle_ok&area_ok&gradient_ok&edge_ok&topology_ok; mode="survey_aware_finite_corridor_v4_adaptive" if membership_available else "survey_aware_anisotropic_v1_membership_unavailable"; aspect_ratio_used=False
        metrics["adaptive_angle_bypass"]=adaptive_angle_bypass; metrics["fails_cross_track_span"]=~cross_ok; metrics["fails_along_track_span"]=~along_ok; metrics["fails_line_topology"]=~topology_ok; metrics["fails_max_angle"]=~effective_angle_ok; metrics["fails_min_area"]=~area_ok; metrics["fails_depth_gradient"]=~gradient_ok; metrics["fails_manual_max_edge"]=~edge_ok; metrics["fails_aspect_ratio"]=False
    else:
        ...
        metrics["along_track_span_m"]=np.nan; metrics["cross_track_span_m"]=np.nan; metrics["triangle_support_class"]="unclassified"; metrics["adaptive_angle_bypass"]=False
        for j in range(3): metrics[f"v{j}_line_id"]=-1; metrics[f"v{j}_line_distance_m"]=np.nan; metrics[f"v{j}_endpoint_distance_m"]=np.nan; metrics[f"v{j}_membership_state"]="unclassified"
    ...
    info={"triangle_qc_version":"survey-aware-v4","triangle_qc_mode":mode,"survey_aware_enabled":bool(survey_aware),"aspect_ratio_used_as_rejection":aspect_ratio_used,"dominant_axis_heading_deg":float(heading) if survey_aware else None,
        ...
        "max_triangle_edge_m":manual_edge_limit,"max_triangle_edge_mode":edge_mode,"max_angle_deg":float(config.max_triangle_angle_deg),"adaptive_adjacent_line_angle_bypass_count":int(adaptive_angle_bypass.sum()),"min_triangle_area_m2":float(config.min_triangle_area_m2),"accepted_triangles":int(mask.sum()),"rejected_triangles":int((~mask).sum()),"rejection_counts":rejection_counts,
        "topology_counts":topology_counts,"line_membership":membership_info,"topology_policy":{"accept":["same_line","adjacent_lines"],"reject":["non_adjacent_lines","multi_line"],"conditional":["line_endpoint_transition","turn_or_transition","unclassified"],"adjacent_line_rule":"finite corridor + anisotropic spans; max-angle bypass allowed"},"defaults":asdict(defaults)}
