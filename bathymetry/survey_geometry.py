...
def estimate_track_geometry(observations: pd.DataFrame, settings: TrackSegmentationConfig|None=None) -> tuple[dict,pd.DataFrame]:
    """Segment ordered survey tracks and estimate survey-line spacing.

    Besides the legacy line centers relative to the moving-observation mean, v2 groundwork
    also reports the same centers in the coordinate frame used by the unique XY surface
    nodes. This lets topology QC assign line membership without mixing large projected
    coordinates with local Delaunay coordinates.
    """
    ...
    if "overall_quality" in accepted.columns:
        accepted=accepted[accepted["overall_quality"].eq("valid_for_surface")]
    ...
    # This exactly matches the default build_surface_points(..., aggregation_mode="none")
    # XY reference: one node per exact projected coordinate pair.
    unique_xy=accepted[["x_m","y_m"]].drop_duplicates().to_numpy(dtype=float)
    surface_origin=np.mean(unique_xy,axis=0)
    ...
    smooth_heading=_smooth_axis_heading(headings,settings.smoothing_window); dominant=_dominant_axis(smooth_heading,lengths)
    ...
    axis=np.array([math.cos(dominant),math.sin(dominant)],dtype=float); normal=np.array([-axis[1],axis[0]],dtype=float)
    origin=np.mean(xy,axis=0); segments=[]
    for segment_id,(edge_start,edge_end) in enumerate(_contiguous_runs(on_axis),start=1):
        point_start=edge_start; point_end=edge_end+1; pts=xy[point_start:point_end+1]
        n_points=len(pts); path_length=float(np.sum(lengths[edge_start:edge_end+1]))
        ...
        segments.append({
            ...
            "point_count":int(n_points),"path_length_m":path_length,"axis_heading_deg":float(math.degrees(dominant)%180.0),
            "cross_track_offset_m":cross_center,"cross_track_mad_m":cross_mad,"along_track_span_m":float(np.ptp(along)),
        })

    tracks=pd.DataFrame(segments)
    if tracks.empty:
        return {"status":"unavailable","reason":"no_straight_survey_segments_detected","input_observations":int(len(ordered)),
                "dominant_axis_heading_deg":float(math.degrees(dominant)%180.0),"typical_step_m":typical_step,"gap_limit_m":gap_limit,"settings":asdict(settings)},tracks
    ...
    # Convert line centers from (point - moving_origin)·normal to
    # (point - unique_surface_origin)·normal. build_local_delaunay uses the latter origin
    # for aggregation_mode="none", so membership is numerically aligned with local_xy.
    surface_shift=float((origin-surface_origin)@normal)
    surface_centers=centers+surface_shift
    summary={
        ...
        "dominant_axis_heading_deg":float(math.degrees(dominant)%180.0),"estimated_line_spacing_m":spacing,"line_spacing_mad_m":spacing_mad,
        "line_spacing_samples":int(len(spacings)),"confidence":confidence,"typical_step_m":typical_step,"gap_limit_m":gap_limit,
        "line_cluster_tolerance_m":cluster_tolerance,"line_centers_cross_track_m":centers.tolist(),
        "surface_reference_origin_x_m":float(surface_origin[0]),"surface_reference_origin_y_m":float(surface_origin[1]),
        "line_centers_surface_cross_track_m":surface_centers.tolist(),"surface_reference_mode":"mean_of_exact_unique_surface_xy",
        "settings":asdict(settings),"method":"ordered_track_segmentation_dominant_axis_cross_track_clusters",
    }
